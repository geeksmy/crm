...
func New(
	...
	return &Server{
		Mounts: []*MountPoint{
			...
			{"UpdatePassword", "POST", "/api/auth/update_password"},
			{"CaptchaImage", "GET", "/api/auth/captcha_image"},
		},
		Login:          NewLoginHandler(e.Login, mux, decoder, encoder, errhandler, formatter),
		UpdatePassword: NewUpdatePasswordHandler(e.UpdatePassword, mux, decoder, encoder, errhandler, formatter),
		CaptchaImage:   NewCaptchaImageHandler(e.CaptchaImage, mux, decoder, encoder, errhandler, formatter),
	}
}
...
func (s *Server) Use(m func(http.Handler) http.Handler) {
	...
	s.CaptchaImage = m(s.CaptchaImage)
}
...
func Mount(mux goahttp.Muxer, h *Server) {
	...
	MountCaptchaImageHandler(mux, h.CaptchaImage)
}
...
// MountUpdatePasswordHandler configures the mux to serve the "Auth" service
...
func MountUpdatePasswordHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("POST", "/api/auth/update_password", f)
}

// NewUpdatePasswordHandler creates a HTTP handler which loads the HTTP request
// and calls the "Auth" service "UpdatePassword" endpoint.
...
// MountCaptchaImageHandler configures the mux to serve the "Auth" service
// "CaptchaImage" endpoint.
func MountCaptchaImageHandler(mux goahttp.Muxer, h http.Handler) {
	f, ok := h.(http.HandlerFunc)
	if !ok {
		f = func(w http.ResponseWriter, r *http.Request) {
			h.ServeHTTP(w, r)
	...
	mux.Handle("GET", "/api/auth/captcha_image", f)
}

// NewCaptchaImageHandler creates a HTTP handler which loads the HTTP request
// and calls the "Auth" service "CaptchaImage" endpoint.
func NewCaptchaImageHandler(
	endpoint goa.Endpoint,
	mux goahttp.Muxer,
	decoder func(*http.Request) goahttp.Decoder,
	encoder func(context.Context, http.ResponseWriter) goahttp.Encoder,
	...
	var (
		encodeResponse = EncodeCaptchaImageResponse(encoder)
		encodeError    = EncodeCaptchaImageError(encoder, formatter)
	)
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := context.WithValue(r.Context(), goahttp.AcceptTypeKey, r.Header.Get("Accept"))
		ctx = context.WithValue(ctx, goa.MethodKey, "CaptchaImage")
		ctx = context.WithValue(ctx, goa.ServiceKey, "Auth")
		var err error
