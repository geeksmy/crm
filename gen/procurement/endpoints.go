// Code generated by goa v3.2.6, DO NOT EDIT.
//
// Procurement endpoints
//
// Command:
// $ goa gen crm/design

package procurement

import (
	"context"

	goa "goa.design/goa/v3/pkg"
	"goa.design/goa/v3/security"
)

// Endpoints wraps the "Procurement" service endpoints.
type Endpoints struct {
	Get    goa.Endpoint
	List   goa.Endpoint
	Update goa.Endpoint
	Create goa.Endpoint
	Delete goa.Endpoint
}

// NewEndpoints wraps the methods of the "Procurement" service with endpoints.
func NewEndpoints(s Service) *Endpoints {
	// Casting service to Auther interface
	a := s.(Auther)
	return &Endpoints{
		Get:    NewGetEndpoint(s, a.JWTAuth),
		List:   NewListEndpoint(s, a.JWTAuth),
		Update: NewUpdateEndpoint(s, a.JWTAuth),
		Create: NewCreateEndpoint(s, a.JWTAuth),
		Delete: NewDeleteEndpoint(s, a.JWTAuth),
	}
}

// Use applies the given middleware to all the "Procurement" service endpoints.
func (e *Endpoints) Use(m func(goa.Endpoint) goa.Endpoint) {
	e.Get = m(e.Get)
	e.List = m(e.List)
	e.Update = m(e.Update)
	e.Create = m(e.Create)
	e.Delete = m(e.Delete)
}

// NewGetEndpoint returns an endpoint function that calls the method "Get" of
// service "Procurement".
func NewGetEndpoint(s Service, authJWTFn security.AuthJWTFunc) goa.Endpoint {
	return func(ctx context.Context, req interface{}) (interface{}, error) {
		p := req.(*GetPayload)
		var err error
		sc := security.JWTScheme{
			Name:           "jwt",
			Scopes:         []string{"role:user", "role:admin"},
			RequiredScopes: []string{"role:user"},
		}
		ctx, err = authJWTFn(ctx, p.Token, &sc)
		if err != nil {
			return nil, err
		}
		res, err := s.Get(ctx, p)
		if err != nil {
			return nil, err
		}
		vres := NewViewedProcurement(res, "default")
		return vres, nil
	}
}

// NewListEndpoint returns an endpoint function that calls the method "List" of
// service "Procurement".
func NewListEndpoint(s Service, authJWTFn security.AuthJWTFunc) goa.Endpoint {
	return func(ctx context.Context, req interface{}) (interface{}, error) {
		p := req.(*ListPayload)
		var err error
		sc := security.JWTScheme{
			Name:           "jwt",
			Scopes:         []string{"role:user", "role:admin"},
			RequiredScopes: []string{"role:user"},
		}
		ctx, err = authJWTFn(ctx, p.Token, &sc)
		if err != nil {
			return nil, err
		}
		return s.List(ctx, p)
	}
}

// NewUpdateEndpoint returns an endpoint function that calls the method
// "Update" of service "Procurement".
func NewUpdateEndpoint(s Service, authJWTFn security.AuthJWTFunc) goa.Endpoint {
	return func(ctx context.Context, req interface{}) (interface{}, error) {
		p := req.(*UpdatePayload)
		var err error
		sc := security.JWTScheme{
			Name:           "jwt",
			Scopes:         []string{"role:user", "role:admin"},
			RequiredScopes: []string{"role:user"},
		}
		ctx, err = authJWTFn(ctx, p.Token, &sc)
		if err != nil {
			return nil, err
		}
		res, err := s.Update(ctx, p)
		if err != nil {
			return nil, err
		}
		vres := NewViewedProcurement(res, "default")
		return vres, nil
	}
}

// NewCreateEndpoint returns an endpoint function that calls the method
// "Create" of service "Procurement".
func NewCreateEndpoint(s Service, authJWTFn security.AuthJWTFunc) goa.Endpoint {
	return func(ctx context.Context, req interface{}) (interface{}, error) {
		p := req.(*CreatePayload)
		var err error
		sc := security.JWTScheme{
			Name:           "jwt",
			Scopes:         []string{"role:user", "role:admin"},
			RequiredScopes: []string{"role:user"},
		}
		ctx, err = authJWTFn(ctx, p.Token, &sc)
		if err != nil {
			return nil, err
		}
		res, err := s.Create(ctx, p)
		if err != nil {
			return nil, err
		}
		vres := NewViewedProcurement(res, "default")
		return vres, nil
	}
}

// NewDeleteEndpoint returns an endpoint function that calls the method
// "Delete" of service "Procurement".
func NewDeleteEndpoint(s Service, authJWTFn security.AuthJWTFunc) goa.Endpoint {
	return func(ctx context.Context, req interface{}) (interface{}, error) {
		p := req.(*DeletePayload)
		var err error
		sc := security.JWTScheme{
			Name:           "jwt",
			Scopes:         []string{"role:user", "role:admin"},
			RequiredScopes: []string{"role:user"},
		}
		ctx, err = authJWTFn(ctx, p.Token, &sc)
		if err != nil {
			return nil, err
		}
		res, err := s.Delete(ctx, p)
		if err != nil {
			return nil, err
		}
		vres := NewViewedSuccessResult(res, "default")
		return vres, nil
	}
}
